...
import {
  ...
  QrCode, ChevronRight, AlertCircle, Loader2, ArrowLeft,
} from 'lucide-react';
...
const AdminPage: React.FC = () => {
  ...
  const [newEvent, setNewEvent] = useState({ id: '', name: '', event_date: '', event_end: '', location: '' });
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    if (newEvent.event_end && newEvent.event_end < newEvent.event_date) {
      setCreateError('La data di fine non può essere precedente alla data di inizio.');
      setCreating(false);
      return;
    }
    ...
    const { error } = await supabase.from('events').insert({
      ...
      event_end: newEvent.event_end || null,
    ...
    setNewEvent({ id: '', name: '', event_date: '', event_end: '', location: '' });
    await loadEvents();
    setCreating(false);
  };
  ...
  const exportCSV = async (eventId: string, eventName: string) => {
    ...
    if (error || !data?.length) {
      alert(data?.length === 0 ? 'Nessuna presenza per questo evento.' : 'Errore nel recupero dati.');
      return;
    }
    ...
    const rows = data.map((a) => {
      ...
      return [
        ...
        p?.dob ?? '',
        a.type,
        new Date(a.scanned_at).toLocaleString('it-IT'),
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 py-10">
        ...
        <form onSubmit={handleCreateEvent} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input type="text" required placeholder="es. jumpin_2026_06"
                className="w-full px-4 py-3 rounded-2xl glass-input text-sm"
                value={newEvent.id}
                onChange={(e) => setNewEvent({ ...newEvent, id: e.target.value })} />
            </div>
            <div className="space-y-1.5">
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Data Inizio *</label>
          ...
          <div className="space-y-1.5">
            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">
              Data Fine <span className="text-gray-300 normal-case font-normal">(opzionale — se l'evento dura più giorni)</span>
            </label>
            <input type="date" className="w-full px-4 py-3 rounded-2xl glass-input text-sm"
              min={newEvent.event_date || undefined}
              value={newEvent.event_end}
              onChange={(e) => setNewEvent({ ...newEvent, event_end: e.target.value })} />
          </div>
          <div className="space-y-1.5">
            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Nome Evento *</label>
            <input type="text" required placeholder="es. JumpIn - Giugno 2026"
              className="w-full px-4 py-3 rounded-2xl glass-input text-sm"
              value={newEvent.name}
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" placeholder="es. Rimini"
              className="w-full px-4 py-3 rounded-2xl glass-input text-sm"
      ...
      <h2 className="text-lg font-bold font-montserrat text-gray-700 mb-4 px-2">
          ...
          {events.map((ev) => (
            <GlassCard key={ev.id}>
              <div className="flex items-start justify-between mb-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-2">
                    <span className="flex items-center gap-1.5 text-xs text-gray-500 font-medium">
                      ...
                      {ev.event_end && ev.event_end !== ev.event_date && (
                        <> → {new Date(ev.event_end).toLocaleDateString('it-IT', { day: '2-digit', month: 'long', year: 'numeric' })}</>
                      )}
                    </span>
                    {ev.location && (
                      <span className="flex items-center gap-1.5 text-xs text-gray-500 font-medium">
                ...
                <button
                  onClick={() => exportCSV(ev.id, ev.name)}
              ...
              <div className="grid grid-cols-2 gap-4">
                {(['ingresso', 'uscita'] as const).map((tipo) => (
                  <div key={tipo} className="flex flex-col items-center p-4 rounded-3xl bg-white/50 border border-white/60">
                    ...
                    <button
                      onClick={() => qrCodes[ev.id] && downloadQR(qrCodes[ev.id][tipo], `qr_${ev.id}_${tipo}.png`)}
