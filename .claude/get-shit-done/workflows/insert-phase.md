...
```bash
INIT=$(node "D:/!programming/botika/jumpInLoginAndQrCode/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "D:/!programming/botika/jumpInLoginAndQrCode/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
