...
```bash
UI_AUDITOR_MODEL=$(node "D:/!programming/botika/jumpInLoginAndQrCode/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-auditor --raw)
```
...
```markdown
Read D:/!programming/botika/jumpInLoginAndQrCode/.claude/agents/gsd-ui-auditor.md for instructions.
...
```bash
node "D:/!programming/botika/jumpInLoginAndQrCode/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
